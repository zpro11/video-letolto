...
if ! command_exists zenity; then
    if command_exists apt-get; then
        echo "Zenity nincs telepítve — telepítés apt-get-tel (sudo szükséges)..."
...
if [ ${#MISSING_APT[@]} -eq 0 ] && [ ${#MISSING_PIP[@]} -eq 0 ]; then
    ...
    for pkg in "${MISSING_APT[@]}"; do
        ...
        if command_exists apt-cache; then
            size_kb=$(apt-cache show "$pkg" 2>/dev/null | awk -F: '/^Installed-Size:/{gsub(/^[ \t]+|[ \t]+$/,"",$2); print $2; exit}')
            if [ -n "$size_kb" ]; then
                total_kb=$((total_kb + size_kb))
            else
                size_bytes=$(apt-cache show "$pkg" 2>/dev/null | awk -F: '/^Size:/{gsub(/^[ \t]+|[ \t]+$/,"",$2); print $2; exit}')
